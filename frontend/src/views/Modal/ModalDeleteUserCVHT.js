...
function ModalDeleteUserCVHT({ setOpenModalDeleteUserCVHT, openModalDeleteUserCVHT, deleteCVHT, doDeleteUserId }) {
    //toggle chi danh cho toggle modal
    const toggle = () => setOpenModalDeleteUserCVHT(!openModalDeleteUserCVHT);
    // console.log(deleteSVCL);
    let deletUserId = async () => {
        try {
            await doDeleteUserId(deleteCVHT);
    ...
    return (
        <Modal isOpen={openModalDeleteUserCVHT} toggle={toggle}>
            <ModalHeader
...
export default ModalDeleteUserCVHT;